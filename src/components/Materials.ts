...
`,`
  ...
  void main () {
    ...
    if (finalColor.r < 0.06 && finalColor.b < 0.06) {
      finalColor.r = 0.1;
      finalColor.g = 0.1;
      finalColor.b = 0.1;
    }
...
`,`
  ...
  void main() {
    ...
    vec4 color = vec4(cosPalette(v_distort, brightness, contrast, oscilation, phase), 1.);
    gl_FragColor = mix(vec4(vec3(0.), 1.), color, clamp(v_distort * 5., 0.8, 1.));
  }
`)
